...
public class ValuationCalculator {
    ...
    public ReportItem calculate(String ticker, SseEmitter sseEmitter) {
        ...
        var peg = calculatePegRatio(ticker);
    ...
    public BigDecimal calculateCurrentPE(String ticker) {
        ...
        return price.divide(ttmEps, 2, RoundingMode.HALF_UP);
    }
    ...
    public BigDecimal calculatePegRatio(String ticker) {
        BigDecimal currentPe = calculateCurrentPE(ticker);
        BigDecimal oneYearForwardEpsGrowth = calculate1YrForwardEpsGrowth(ticker);

        if (currentPe == null) {
            LOGGER.warn("Current PE is null for PEG ratio calculation for ticker: {}", ticker);
            return null;
        }
        ...
        return currentPe.divide(oneYearForwardEpsGrowth, 2, RoundingMode.HALF_UP);
    }
}